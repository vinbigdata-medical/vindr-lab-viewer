import { import as csTools, toolColors } from 'vindr-tools';
...
const convertToVector3 = csTools('util/convertToVector3');
...
export default function(
  ...
  const targetNormal = tRowCosines.clone().cross(tColCosines);
  const referenceNormal = rRowCosines.clone().cross(rColCosines);
  ...
  draw(context, context => {
    drawLine(context, targetElement, referenceLine.start, referenceLine.end, {
      color,
      lineWidth: 2,
    });
  });
