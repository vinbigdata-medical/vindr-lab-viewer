...
export default function(vtkVolume, options = {}) {
  ...
  const ippVec3 =
    ipp === 'center' ? vtkVolume.centerIpp : ipp.split(',').map(parseFloat);
  ...
  const newIpp = convertIPP(
    [axes[12].toFixed(6), axes[13].toFixed(6), axes[14].toFixed(6)],
    [
      vtkVolume.centerIpp[0].toFixed(6),
      vtkVolume.centerIpp[1].toFixed(6),
      vtkVolume.centerIpp[2].toFixed(6),
    ]
  );
  ...
  const result = {
    ...
    metaData: {
      imagePlaneModule: {
        ...
        rows: dimensions[1],
        originIPP: [
          parseFloat(vtkVolume.centerIpp[0].toFixed(6)),
          parseFloat(vtkVolume.centerIpp[1].toFixed(6)),
          parseFloat(vtkVolume.centerIpp[2].toFixed(6)),
        ],
      },
    },
  };
...
const convertIPP = (currentIPP = [], originIPP = []) => {
  const tempIpp = [0, 0, 0];
  const cX = `${originIPP[0]}`.split('.');
  const cY = `${originIPP[1]}`.split('.');
  const cZ = `${originIPP[2]}`.split('.');

  const ippFloorX = `${currentIPP[0]}`.split('.')[0];
  const ippFloorY = `${currentIPP[1]}`.split('.')[0];
  const ippFloorZ = `${currentIPP[2]}`.split('.')[0];

  if (cX.length > 1) {
    tempIpp[0] = parseFloat(parseFloat(`${ippFloorX}.${cX[1]}`).toFixed(6));
  } else {
    tempIpp[0] = parseFloat(parseFloat(`${ippFloorX}`).toFixed(6));
  }
  if (cY.length > 1) {
    tempIpp[1] = parseFloat(parseFloat(`${ippFloorY}.${cY[1]}`).toFixed(6));
  } else {
    tempIpp[1] = parseFloat(parseFloat(`${ippFloorY}`).toFixed(6));
  }
  if (cZ.length > 1) {
    tempIpp[2] = parseFloat(parseFloat(`${ippFloorZ}.${cZ[1]}`).toFixed(6));
  } else {
    tempIpp[2] = parseFloat(parseFloat(`${ippFloorZ}`).toFixed(6));
  }
