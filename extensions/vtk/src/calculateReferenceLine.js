import { import as csTools } from 'vindr-tools';
...
export default function(targetImagePlane, referenceImagePlane) {
  const patientPoint = planePlaneIntersection(
    targetImagePlane,
    referenceImagePlane
  );
  ...
  const start = projectPatientPointToImagePlane(
    patientPoint.start,
    targetImagePlane
  );
  const end = projectPatientPointToImagePlane(
    patientPoint.end,
    targetImagePlane
  );
