...
import {
  actionPreloadNextStudies,
  initLoadedStudy,
  getLoadedStudy,
  setLoadedStudy,
} from './ViewerPreloadStudyData';
...
function ViewerRetrieveStudyData({
      ...
      const studies = studiesData.map(study => {
        ...
        cancelableSeriesPromises[study.StudyInstanceUID] = makeCancelable(
          ...
          .then(result => {
            ...
            handleLoadNextStudy(study.StudyInstanceUID);
          })
          .catch(error => {
  ...
  const handleLoadNextStudy = currentUID => {
    setTimeout(() => {
      if (listStudyUIDs.length > 0) {
        const loadedStudies = getLoadedStudy();
        const idx = (listStudyUIDs || []).indexOf(currentUID);
        if (
          idx >= 0 &&
          idx <= listStudyUIDs.length - 2 &&
          !loadedStudies[listStudyUIDs[idx + 1]]
        ) {
          actionPreloadNextStudies(
            server,
            [listStudyUIDs[idx + 1]],
  ...
  useEffect(() => {
    if (studyInstanceUIDs.length > 0) {
      ...
      cancelableStudiesPromises = {};
      const loadedStudies = getLoadedStudy();
      if (loadedStudies[studyInstanceUIDs[0]]) {
        setStudies(loadedStudies[studyInstanceUIDs[0]]);
        forceRerender();
        handleLoadNextStudy(studyInstanceUIDs[0]);
      } else {
