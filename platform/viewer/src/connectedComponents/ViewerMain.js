...
class ViewerMain extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    const hasStudyChanged = this.props.studies !== prevProps.studies;

    if (hasStudyChanged || (viewportAmount !== prevViewportAmount && !isVtk)) {
      const displaySets = this.getDisplaySets(this.props.studies);
      this.setState({ displaySets }, () =>
        this.fillEmptyViewportPanes(hasStudyChanged)
      );
    }
  }

  fillEmptyViewportPanes = hasStudyChanged => {
  ...
  setViewportData = ({
    ...
    if (displaySet) {
      if (!displaySet.SOPInstanceUID) {
        const imageId = displaySet.images[0].SOPInstanceUID;
        displaySet.SOPInstanceUID = imageId;
      }
