...
import WindowLevelSettings from '../components/WindowLevelSettings';

class ToolbarRow extends Component {
  // TODO: Simplify these? isOpen can be computed if we say "any" value for selected,
  // closed if selected is null/undefined
  static propTypes = {
    isLeftSidePanelOpen: PropTypes.bool,
    isRightSidePanelOpen: PropTypes.bool,
    selectedLeftSidePanel: PropTypes.string,
  ...
  constructor(props) {
    ...
    const activeButtons =
      get(this.props, 'extensions.toolbar.activeButtons') || [];
  ...
  componentDidUpdate(prevProps) {
    ...
    if (activeContextsChanged) {
      const buttons = _getVisibleToolbarButtons.call(this);

      buttons.forEach(function(item, i) {
        if (item.id === 'TagBrowser') {
          buttons.splice(i, 1);
    ...
    const currentActiveButtons = get(
      this.props,
      'extensions.toolbar.activeButtons'
    );
    const prevActiveButtons = get(
      prevProps,
      'extensions.toolbar.activeButtons'
    );
    if (currentActiveButtons !== prevActiveButtons) {
      this.setState({ activeButtons: currentActiveButtons });
    }
  }
...
function _handleToolbarButtonClick(button, evt, props) {
  if (button) {
    this.props.actionSetSelectedTool([button]);
  }
...
export default connect(
  ...
  withTranslation(['Common', 'ViewportDownloadForm'])(
